...
public class SocketManager implements IEvent {
    ...
    public void connect(String url, String userId, int device, UsbSerialThread thread) {
        if (webSocket == null || !webSocket.isOpen()) {
            ...
            webSocket = new MyWebSocket(uri, this, thread);
            // 设置wss
    ...
    public void sendSerialPortCmd(String roomId, int cmd, int up, int level, int angle) {
        if (webSocket != null) {
            try {
                webSocket.sendSerialPortCmd(cmd, roomId, up, level,angle);
            } catch (Exception e) {
